...
Rcpp::List saem_fit_mixture_a_model(Rcpp::List curve_list,
  ...
  for(Rcpp::List::iterator it = curve_list.begin(); it != curve_list.end(); ++it) {
    data->push_back(Mixture_A_Curve(*it, pars, id, seed));
    ++id;
  }
...
#pragma omp parallel for
  ...
  Rcpp::Rcout << "return_list" << std::endl;
  for(int i = 0; i < pars->n_curve; ++i){
    curves[i] = data->at(i).return_list(y_scaling_factor);
  }
  Rcpp::Rcout << "pars->return_pars" << std::endl;
  est_pars = pars->return_pars(y_scaling_factor);
  Rcpp::Rcout << "pars->return_aux" << std::endl;
  aux = pars->return_aux();
  Rcpp::Rcout << "pars->return_pars_tracker" << std::endl;
