...
// class_pars.h
#ifndef CLASS_MIXED_WARPING_MODEL_H
#define CLASS_MIXED_WARPING_MODEL_H
...
public:
  ...
  // Dimension
  int dim_f;     // number of basis coefficients of the base shape
  int dim_a;     // dimension of amplitude effect, (=2)
  int dim_w;     // number of basis coefficient of the warping function
  ...
  int f_order;                    // order of the base shape splines
  int h_order;                    // order of the warping function splines
  RcppGSL::Vector f_break_points; // boundary and internal knot locations of the base shape splines
  ...
  // Constructor
  Mixed_Warping_Model(Rcpp::List pars_list,
  ...
  void update_estimates_data_mod(std::vector<Mixed_Warping_Curve>* mydata);
  void update_estimates_amp_mod(std::vector<Mixed_Warping_Curve>* mydata);
  void update_estimates_single_warp_mod(std::vector<Mixed_Warping_Curve>* mydata);
  void initialize_clustering_with_user_inputs(std::vector<Mixed_Warping_Curve>* mydata);
  void update_estimates_mixture_warp_mod(std::vector<Mixed_Warping_Curve>* mydata);
  void update_fisher_information_approx(std::vector<Mixed_Warping_Curve>* mydata);
