...
//' @noRd
// [[Rcpp::export]]
Rcpp::List saem_fit_mixed_warping(Rcpp::List curve_list,
                                  Rcpp::List pars_list,
  ...
  // Import parameters
  Mixed_Warping_Model* pars = new Mixed_Warping_Model(pars_list, control_list,
                                                      f_break_points, h_break_points);
  pars->generate_chol_centering_mat();

  // Import data
  std::vector<Mixed_Warping_Curve>* data = new std::vector<Mixed_Warping_Curve>;
  int id = 0;
  for(Rcpp::List::iterator it = curve_list.begin(); it != curve_list.end(); ++it) {
    data->push_back(Mixed_Warping_Curve(*it, pars, id, seed));
    ++id;
  }

  // Initialize basis evaluation matrices
  for(std::vector<Mixed_Warping_Curve>::iterator it = data->begin(); it != data->end(); ++it){
